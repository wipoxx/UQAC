...
class TrajetElementaire extends Hydratable{
    ...
    public static function getWithId($idMin, $idMax) {
        $query = "SELECT * FROM trajetelementaire WHERE IdTrajetElementaire BETWEEN :idMin AND :idMax;";
        $parameters = array( array('name' => ':idMin', 'value' => $idMin, 'type' => 'string'), 
                             array('name' => ':idMax', 'value' => $idMax, 'type' => 'string')
                           );
        $results = null;
        $db = BDDLocale::getInstance();
        if($db->get($query, $results, $parameters))
        {
            ...
            foreach($results as $result) {
                $lTrajets[] = new TrajetElementaire($result);
            }
            return $lTrajets;
        }
    }
